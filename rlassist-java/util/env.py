...
from util.helpers import get_rev_dict, split_list, tokens_to_source, make_dir_if_not_exists, prepend_line_numbers, prepare_batch, compilation_errors, coin_flip, java_tokens_to_source
...
class Env_engine:
    ...
    def get_normalized_ids_dict(self, tl_dict):
        ...
        normalized_ids_tl_dict['EOF'] = self.cursor           # 1
        id_token_vecs = []
        ...
        normalized_ids_tl_dict['IDENTIFIER@'] = len(normalized_ids_tl_dict)
    ...
    def __init__(self, tl_dict, seed, correct_fix_reward=1.0, step_penalty=-0.01,
        ...
        self.action_tokens = ['LPAREN', 'RPAREN', 'COMMA', 'SEMI', 'LBRACE', 'RBRACE', 'DOT']
        ...
        self.replacement_action_to_action_sequence_map = {
            'replace;)with);': ['delete SEMI RPARE', 'move_right', 'insert SEMI'],
        ...
        # 限定了能够修改的token
        self.mutables = [self.tl_dict[each]
                         for each in self.action_tokens]
    ...
    def normalize_ids(self, prog_vector):
        normalized_id = self.normalized_ids_tl_dict['IDENTIFIER@']
    ...
    def normalize_id(self, token):
        if token in self.id_token_vecs:
            return self.normalized_ids_tl_dict['IDENTIFIER@']
    ...
    def act(self, program, cursor, action):
        ...
        # 如果是insert的话，原来通过action[-1]找到操作的token
        # 然后使用'_<op>_' + action[-1]来变成tokenizer后的格式
        if 'insert' in action:
        ...
        if 'replace' in action:
            if 'replace;)with);' in action:
                try:
                    if self.rev_tl_dict[program[cursor]] == 'SEMI' and self.rev_tl_dict[program[cursor + 1]] == 'RPAREN':
                        program, cursor = self.act(program, cursor, 'delete SEMI')
                        program, cursor = self.act(
                            program, cursor, 'move_right')
                        return self.act(program, cursor, 'insert SEMI')
                except:
                ...
                # 这里action[7]是准备删除的
                # action[-1]是准备添加的
                equiv_actions = self.replacement_action_to_action_sequence_map[action]
                token_to_delete = equiv_actions[0].split()[1]
                token_to_insert = equiv_actions[1].split()[1]
                if self.rev_tl_dict[program[cursor]] == token_to_delete:
                    program, cursor = self.act(
                        program, cursor, 'delete' + ' ' + token_to_delete)
                    return self.act(program, cursor, 'insert' + ' ' + token_to_insert)
            return program, cursor
    ...
    def show(self, program_id, program_vector, name_dict, name_seq, original_program_vector, use_compiler):
        ...
        err_list = self.get_javac_parse_errors(program_id, program_vector, name_dict, name_seq) if use_compiler else self.get_errors_from_ground_truth(
    ...
    def get_javac_parse_errors(self, program_id, program, name_dict, name_seq):
        '''
        使用javac parser来得到java程序的错误数量
        '''
        program_source = self.get_java_source_from_vector(program, name_dict, name_seq)
        
        # 调用javac_parser，得到err_list
        
        err_list = self.java.check_syntax(program_source)

        return err_list

    def get_java_source_from_vector(self, program_vector, name_dict, name_seq, keep_cursor=False, clang_format=False, get_tokens=False):
        '''
        将vector还原为java程序源代码
        '''
        # tokens_to_source()接受的是devectorize后的程序 + name_dict
        # 通过devectorize(program_vector, keep_cursor)来得到devectorize后的程序
        # devectorize(program_vector, keep_cursor)不需要修改
        # 那就只需要修改tokens_to_source 这个文件在helpers里面
        return java_tokens_to_source(self.devectorize(program_vector, keep_cursor), name_dict)
    ...
    def update(self, program_id, program_vector, name_dict, name_seq, action, \
        ...
        if 'move_' not in action_name:
            # 如果move_不再action_name中，则说明要对程序作出修改，则需要判断error_list
            reward += self.edit_penalty
            # 只要作出edits 就会又penalty
            err_list = self.get_javac_parse_errors(program_id, updated_program_vector, name_dict, name_seq)

            if len(err_list) == 0:
                # 如果没有error了，则加上reward
                halt = True
                reward += self.correct_fix_reward
            else:
                # 得到原来程序的错误列表
                old_err_list = self.get_javac_parse_errors(program_id, program_vector, name_dict, name_seq)
                # 计算correction
                corrections = len(old_err_list) - len(err_list)
                # 这个corrections是原来error数量减去新的error数量，即看有没有减少error数量
                if corrections > 0:
                    # 大于0 表示减少了数量
                    reward += self.error_resolution_reward
                if reject_edits and corrections <= 0:
                    # <=0 表示没有修复error，如果reject_edits的话，则并不更新，还是返回原来的东西
    ...
    def new_random_episode_with_right_actions(self, program_id, cursor_program, name_dict, name_seq, original_program, org_err_cnt, use_compiler, toy=False):
        ...
        final_errs = self.get_javac_parse_errors(program_id, current_program, name_dict, name_seq) if not toy and use_compiler else \
            self.get_errors_from_ground_truth(current_program, original_program)
        if len(final_errs) == 0 and not toy and use_compiler:
            init_errs = len(self.get_javac_parse_errors(program_id, cursor_program, name_dict, name_seq))
...
class Environment(Env_engine):
    ...
    def get_identifiers(self, code_id):
        try:
            if '_v' in code_id:
                code_id = code_id.split('_')[0] + '_' + code_id.split('_')[1]
    ...
    def new_indexed_episode(self, index, use_compiler, which='train', code_id=None):
        ...
        # 这里我们修改为从javac_parser获得错误信息
        use_compiler = True
        error_count = len(self.get_javac_parse_errors(code_id, program, name_dict, name_seq)) if use_compiler else self.get_errors_from_ground_truth(program, original_program)
    ...
    def new_random_episode(self, use_compiler, which, GE_selection_probability):
        ...
        name_dict, name_seq = self.get_identifiers(code_id)
        program, original_program = copy.deepcopy(data[code_id])
        error_count = len(self.get_javac_parse_errors(
    ...
    def __init__(self, data_folder, load_real_test_data=False, load_seeded_test_data=False, load_only_dicts=False, shuffle=False, seed=1189):
        ...
        try:
            ...
            self.test_name_dict_store = np.load(os.path.join(
                data_folder, 'test_name_dict_store.npy'), allow_pickle=True).item()
            self.name_dict_store = dict(self.name_dict_store, **self.test_name_dict_store)
        except:
